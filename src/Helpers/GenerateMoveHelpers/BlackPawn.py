...
from src.Root.Const import before_top_edge, before_bottom_edge
...
from src.Helpers.PreCalculationHelpers.BitManipulationHelpers import get_least_bit_index, move_bit_by_position
from src.Root.PreCalculationsData import square_bitmask, pawn_attack_maps


def get_black_pawn_moves(move_model: GameState) -> list[int]:
    ...
    while pawn and move_model.attack_on_king_attr.check_count < 2:
        ...
        quite_move: int = move_bit_by_position(pawn_positional_mask, Direction.SOUTH)
        ...
        if pawn_positional_mask & before_top_edge:
            double_push_move: int = move_bit_by_position(quite_move, Direction.SOUTH)
